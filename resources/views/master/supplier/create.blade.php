...
@section('breadcrumb', 'Tambah Supplier')
@section('content')
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Tambah Supplier</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Kode supplier dibuat otomatis oleh sistem.</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800"><h3 class="text-sm font-semibold text-slate-900 dark:text-white">Informasi Supplier</h3></div>
                    <div class="px-6 py-5 space-y-4">

                        {{-- Kode auto --}}
                        <div class="space-y-1.5">
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Kode Supplier</label>
                            <div class="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg">
                                ...
                                <span class="text-sm font-mono text-slate-500 dark:text-slate-400">Dibuat otomatis (SUP-XXXXX)</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" id="nama_supplier" name="nama_supplier" value="{{ old('nama_supplier') }}" required placeholder="Nama perusahaan / peternakan"
                                   class="block w-full px-3 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors @error('nama_supplier') border-red-400 @enderror">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="space-y-1.5">
                                ...
                                <input type="text" id="nama_kontak" name="nama_kontak" value="{{ old('nama_kontak') }}" placeholder="Contact person"
                                       class="block w-full px-3 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors">
                            </div>
                            <div class="space-y-1.5">
                        ...
                        <div class="flex items-center gap-3 px-3 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg">
                            ...
                            <div><p class="text-[11px] text-slate-400">Saldo Hutang</p><p class="text-xs text-slate-500">Diperbarui otomatis oleh modul Pembelian dan Laporan</p></div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="px-5 py-4">
                        <div x-data="{ enabled: true }">
                            ...
                            <div class="flex items-center justify-between">
                                <div><p class="text-sm font-medium text-slate-700 dark:text-slate-300">Status Aktif</p><p class="text-xs text-slate-400" x-text="enabled ? 'Supplier aktif' : 'Supplier tidak aktif'"></p></div>
